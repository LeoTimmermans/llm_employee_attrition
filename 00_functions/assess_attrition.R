...
calculate_attrition_cost <- function(
  ...
  productivity_cost <- net_revenue_per_employee /
    workdays_per_year *
    (workdays_position_open + workdays_onboarding * onboarding_efficiency)

  # Savings of Salary & Benefits (Cost Reduction)
  salary_benefit_reduction <- salary /
    workdays_per_year *
    workdays_position_open

  # Estimated Turnover Per Employee
  cost_per_employee <- direct_cost +
    productivity_cost -
...
plot_attrition <- function(
  ...
  units_val <- switch(units[[1]], "M" = 1e6, "K" = 1e3, "0" = 1)
  if (units[[1]] == "0") {
    units <- ""
  }
  ...
  data_manipulated <- data %>%
    ...
    mutate(
      value_text = str_c(usd(!!value_expr / units_val), units[[1]], sep = "")
    )
